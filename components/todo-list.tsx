...
export function TodoList() {
  ...
  const toggleTask = async (id: string) => {
    ...
    // Optimistic update — keep pending first, completed after
    setTasks((prev) =>
  ...
  return (
    <div
      className="relative w-[306px] h-80 mx-auto flex items-center justify-center select-none rounded-[28px] p-4 group"
      style={{
    ...
    >
      ...
      <div className="relative w-full h-full flex items-center justify-center scale-90">
        ...
        {/* Front paper — hover animates ONLY this paper, not the container */}
        <motion.div
          className="relative bg-[#fff6e5] rounded-[5px] flex flex-col pt-5 pb-4 px-5"
          style={{ width: 210, height: 240, boxShadow: "3.5px 3.5px 3.5px rgba(0,0,0,0.25)" }}
          animate={{ scale: 1, rotate: 0, y: 0 }}
          whileHover={{ scale: 1.02, rotate: 1, y: -2, boxShadow: "5px 5px 8px rgba(0,0,0,0.2)" }}
          transition={{ type: "spring", stiffness: 300, damping: 20 }}
        >
          {/* Header: title + pending badge */}
          <div className="flex items-center justify-between mb-2 shrink-0">
            <span
              className="text-[10px] font-bold tracking-widest uppercase text-[#2664eb]/50"
              style={{ fontFamily: "var(--font-geist-sans, sans-serif)" }}
            >
              Tasks
            </span>
            {!loading && !notConnected && pendingCount > 0 && (
              <span className="text-[10px] font-semibold text-[#2664eb]/40">
                {pendingCount} left
              </span>
            )}
          </div>

          {/* Task list — stopPropagation on wheel so motion.div hover and parent containers don't swallow scroll */}
          <div
            className="flex-1 w-full flex flex-col gap-1.5 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] relative"
          ...
          {/* Scroll fade — visible when there are enough tasks to scroll */}
          {tasks.length > 4 && (
            <div
              className="pointer-events-none absolute bottom-[52px] left-0 right-0 h-6 rounded-b-[5px]"
              style={{ background: "linear-gradient(to bottom, transparent, #fff6e5)" }}
            />
          )}
          ...
          {!notConnected && (
            <form
              onSubmit={addTask}
              className="relative z-10 flex items-center gap-2 mt-2 border-t border-[#2664eb]/20 pt-2 shrink-0"
            >
              <input
                ref={inputRef}
                type="text"
              ...
              <button
                ...
                className="text-[#2664eb] hover:bg-[#2664eb]/10 rounded-full p-1.5 transition-all active:scale-90 hover:scale-110 disabled:opacity-30 disabled:hover:scale-100 disabled:cursor-not-allowed"
                aria-label="Add task"
              >
                <Plus size={18} />
              </button>
